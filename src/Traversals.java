...
public class Traversals {
  ...
  public static int sumLeafNodes(TreeNode<Integer> node) {
    ...
    int sumLeaf = leftLeaf + rightLeaf;
  ...
  public static int countInternalNodes(TreeNode<Integer> node) {
    ...
    int countLeft = countInternalNodes(node.left);
    int countRight = countInternalNodes(node.right);
  ...
  // helper function 
  private static void countDistinctValues(TreeNode<Integer> node, Set<Integer> set){
    if (node == null) return;
  ...
  public static boolean hasStrictlyIncreasingPath(TreeNode<Integer> node) {
    ...
    if (node.left != null) {
      if (node.left.value > node.value && hasStrictlyIncreasingPath(node.left)) return true;
    }

    if (node.right != null) {
      if (node.right.value > node.value && hasStrictlyIncreasingPath(node.right)) return true;
    }
